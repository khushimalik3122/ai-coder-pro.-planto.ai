...
export async function git_commit(args: { message: string }): Promise<ToolResult> {
  const ws = vscode.workspace.workspaceFolders?.[0]; if (!ws) return { ok:false, error:'No workspace' };
  await sh(ws.uri.fsPath, 'git add -A');
  return sh(ws.uri.fsPath, `git commit -m ${JSON.stringify(args.message || 'AI iteration')}`);
}
export async function git_revert(args: { commit?: string }): Promise<ToolResult> {
  const ws = vscode.workspace.workspaceFolders?.[0]; if (!ws) return { ok:false, error:'No workspace' };
  const ref = args.commit ? args.commit : 'HEAD~1';
